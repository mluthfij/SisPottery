...
(1..100).each do |i|
    Product.create(name: "Product #{i}", user_id: 6, price: rand(15..100), created_at: Time.now - i.days, updated_at: Time.now - i.days)
end
...
(1..100).each do |k|
    # Keep.create(product_name: "Product #{i}", user_id: 1, product_price: rand(15..100), point: i, quantity: rand(1..50), created_at: Time.now - i.days, updated_at: Time.now - i.days)
    # Keep.create(product_name: "Product #{i}", user_id: 1, product_price: rand(15..100), point: i, quantity: rand(1..50), created_at: Time.now - i.days, updated_at: Time.now - i.days)
    Keep.create(bucket_id: 6, customer: "luthfi", product_name: "Product #{k}", product_price: rand(500000..2500000), quantity: rand(5..50), total_price: rand(500000..2500000), order_start: "0026-10-22", order_end: "2022-10-05", point: k, product_id: k, user_id: 7, created_at: Time.now - k.days, updated_at: Time.now - k.days)
end