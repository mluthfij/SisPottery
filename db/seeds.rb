...
#     (1..100).each do |i|
#         Product.create(name: "Product #{i}", user_id: 1, price: rand(500000..1000000), created_at: Time.now - i.days, updated_at: Time.now - i.days)
#     end
# end
...
# keeps.each do |keep|
#     # (1..5).each do |k|
#         keep.update(created_at: Time.now - rand(1..5).days, updated_at: Time.now - rand(1..5).days, quantity: rand(1..10))
#     # end
# end